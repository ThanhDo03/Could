...
class UserController extends Controller
{
    ...
    public function payment(Request $request){
        ...
        if(!$user){
            ...
            if($user->usd < $request->sum){
                $mes = "Not enough Usd";
            }
            else{
                DB::table('uvalue') -> where('uid', $uid)->decrement('usd', $request->sum);
                $confirm = DB::table('cart')->where('uid', $uid)->update(['buyed' => true]);
                $mes = "Successful!";
            }
        }

        return redirect()->route('cart',compact('mes','usercart'));
    }
